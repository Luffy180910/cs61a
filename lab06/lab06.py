...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        make_change_result = make_change(coin, self.coins)
        if make_change_result is None:
            return [coin]
        for c in make_change_result:
            self.coins[c] = self.coins[c]-1
            if self.coins[c] == 0:
                del self.coins[c]
