...
def digit_distance(n):
    ...
    if n<=9:
        return 0
    dig_10= (n % 100 - n % 10) // 10
    dig_1 = n % 10
    distance = abs(dig_1 - dig_10)
...
def count_coins(total):
    ...
    def f(total , max_coins):
        ...
        if total<0:
            return 0
        return f(total-max_coins,max_coins) + f(total,next_smaller_coin(max_coins)) 
    return f(total,25)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda x:x(x))(lambda x : lambda n: 1 if n==1 else n*x(x)(n-1))(10)
